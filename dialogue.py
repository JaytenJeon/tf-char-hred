...
class Dialogue:
    ...
    def next_batch(self, num_context, num_utterance):
        enc_batch = []
        dec_batch = []
        target_batch = []
        enc_seq_length = []
        ...
        for i in range(num_context):
            enc, dec, trg, enc_seq_len, dec_seq_len, dec_max_len = self.next_context(num_utterance)
            enc_batch.append(enc)
            dec_batch.append(dec)
            target_batch.append(trg)
            enc_seq_length.append(enc_seq_len)
            dec_seq_length.append(dec_seq_len)
            dec_max_length.append(dec_max_len)

        return enc_batch, dec_batch, target_batch, enc_seq_length, dec_seq_length, dec_max_length

    def next_context(self, batch_size):
        enc_batch = []
        dec_batch = []
        target_batch = []
        enc_seq_len = []
